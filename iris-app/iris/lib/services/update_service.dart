...
class UpdateService {
  ...
  static Future<void> checkForUpdates(BuildContext context, {bool forceCheck = false}) async {
    try {
      ...
      final lastCheck = DateTime.tryParse(prefs.getString(_lastUpdateCheckKey) ?? '');

      // Skip if we checked recently and it's not a forced check
      if (!forceCheck && lastCheck != null && DateTime.now().difference(lastCheck) < _checkInterval) {
        return;
      }
      ...
      if (response.statusCode == 200) {
        ...
        final latestVersion = releaseData['tag_name']?.replaceFirst('v', '') ?? '0.0.0';
